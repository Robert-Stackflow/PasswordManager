#include "controller/navlistview.h"
#include "qpainter.h"
#include "qfile.h"
#include "qdom.h"
#include "qdebug.h"
...
QSize NavDelegate::sizeHint(const QStyleOptionViewItem &, const QModelIndex &index) const
{
...
QModelIndex NavModel::getModelIndex(int row,int column)
{
    return NavModel::createIndex(row,column,nullptr);
}
QModelIndex NavModel::getModelIndex(const QString& nodeData)
{
    for(int i=0;i<listNode.size();i++)
        if(listNode[i].treeNode->label==nodeData)
            return NavModel::createIndex(i,0,nullptr);
    return NavModel::createIndex(-1,0,nullptr);
}
...
QVariant NavModel::data(const QModelIndex &index, int role) const
{
    ...
    if (role == Qt::DisplayRole) {
        ...
        return QVariant::fromValue(listNode[index.row()].treeNode);
    }
    return QVariant();
}
...
QModelIndex NavModel::parentModelIndex(const QModelIndex &index) const
{
    if(listNode[index.row()].treeNode->children.size()!=0)
        return index;
    for(int i=index.row();i>=0;i--)
        if(listNode[i].treeNode->children.size()>0)
            return NavModel::createIndex(i,0);
    return index;
}
...
NavModel *NavListView::getModel() const
{
    return model;
}
void NavListView::setCurrentIndexByData(const QString& itemName)
{
    this->setCurrentIndex(model->getModelIndex(itemName));
    emit customCurrentIndexChanged(model->getModelIndex(itemName));
}
