...
class NavModel : public QAbstractListModel
{
    Q_OBJECT
public:
    QModelIndex parentModelIndex(const QModelIndex &index)const;
...
public slots:
    QModelIndex getModelIndex(const QString& nodeData);
    QModelIndex getModelIndex(int row,int column=0);
...
public:
    QModelIndex parentModelIndex(const QModelIndex& index)const{
        return model->parentModelIndex(index);
    }
    ...
    NavModel *getModel() const;

public slots:
    //设置当前索引
    void setCurrentIndexByData(const QString& itemName);
    //读取xml文件数据
...
signals:
    void customCurrentIndexChanged(const QModelIndex&);
};
